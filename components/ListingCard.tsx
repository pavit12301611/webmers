...
export default async function ListingCard({ listing }: { listing: Listing }) {
  ...
  return (
    <Link
      ...
      className="group relative block overflow-hidden rounded-[1.8rem] bg-[#121212] ring-1 ring-white/[0.06] transition-all duration-500 hover:-translate-y-1 hover:ring-white/15"
    >
      {/* Thumbnail - only place where full color is allowed (like original brief) */}
      <div className="relative aspect-[4/3] overflow-hidden">
        <div className="absolute inset-0 transition-transform duration-[800ms] group-hover:scale-[1.04]">
      ...
      <div className="flex items-center justify-between px-5 py-3.5 text-[12px]">
        ...
        <span className="text-[11px] uppercase tracking-[0.14em] text-white/30 group-hover:text-white/60 transition">
          View →
        </span>
      </div>
    </Link>
