...
export default function Header() {
  ...
  return (
    <>
        ...
        <div className="relative flex items-center justify-between px-5 pt-5 md:px-8">
          ...
          >
            <svg
              ...
              height="28"
              fill="white"
              className="drop-shadow-[0_1px_8px_rgba(255,255,255,0.3)]"
              aria-hidden="true"
            >
            ...
            <span className="hidden md:inline font-medium tracking-tight text-white text-[13px] uppercase">Webmers</span>
          </Link>

          {/* Desktop center pill nav */}
          <nav className="pointer-events-auto hidden md:flex fixed left-1/2 top-5 -translate-x-1/2 items-center gap-1 rounded-full px-2 py-1.5 liquid-glass">
            {NAV_ITEMS.map((item) => (
              <Link
                key={item.label}
                href={item.href}
          ...
          <button
            ...
            className="pointer-events-auto md:hidden liquid-glass flex h-11 w-11 items-center justify-center rounded-full"
      ...
      <div
        className={`fixed inset-0 z-[55] bg-[#0a0a0a] transition-transform duration-500 ease-[cubic-bezier(0.77,0,0.18,1)] ${
      ...
      >
        ...
        <div className="flex h-[calc(100vh-80px)] flex-col items-center justify-center gap-6 px-6 pb-20">
          <nav className="flex flex-col items-center gap-5">
            {MOBILE_NAV.map((item, idx) => (
              <Link
                ...
                style={{
                  animationDelay: isOpen ? `${100 + idx * 60}ms` : '0ms',
                }}
              >
                {item.label}
              </Link>
            ...
            {user ? (
              <Link
                href={dashboardHref(user.role)}
                onClick={() => setIsOpen(false)}
                className={`mt-2 text-3xl font-medium text-white/90 sm:text-4xl ${
                  isOpen ? 'animate-slide-up' : 'opacity-0'
                }`}
                style={{ animationDelay: isOpen ? `${100 + MOBILE_NAV.length * 60}ms` : '0ms' }}
              >
                Dashboard
              </Link>
            ) : null}
          ...
          <div
            ...
            style={{ animationDelay: isOpen ? `${100 + (MOBILE_NAV.length + 1) * 60}ms` : '0ms' }}
          >
            {user ? (
              <button
                onClick={() => {
