  ...
  return (
    <main className="bg-background text-foreground px-6 md:px-10 py-20 md:py-28">
      <div className="mx-auto max-w-4xl">
        ...
        <p className="text-[15px] leading-7 text-foreground/45 mb-14 max-w-2xl">Real-time monitoring, structured logs, and system status for the Webmers platform.</p>
          ...
          {[
            ...
            <div key={s.label} className="rounded-[1.6rem] border border-white/[0.08] bg-white/[0.03] p-6 backdrop-blur-xl">
              <div className="flex items-center gap-3 mb-3">
                ...
                <span className="text-xs text-foreground/30 uppercase tracking-widest">{s.label}</span>
              </div>
              <div className="text-2xl font-display font-bold mb-1">{s.value}</div>
              <div className="text-[11px] text-emerald-700">{s.status}</div>
            </div>
          ))}
        </div>
      </div>
