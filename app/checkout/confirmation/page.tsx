    ...
    <main className="relative min-h-screen">
      ...
      <div className="min-h-screen flex items-center justify-center px-6 py-28">
        <div className="max-w-lg w-full text-center">
          ...
          <p className="text-xl text-foreground/40 mb-8">
          ...
          {order && (
            <div className="p-6 rounded-3xl bg-white/[0.04] border border-white/[0.08] mb-8 text-left">
              ...
              <dl className="space-y-2 text-sm">
                <div className="flex justify-between"><dt className="text-foreground/40">Website</dt><dd className="font-medium">{order.listingTitle}</dd></div>
                <div className="flex justify-between"><dt className="text-foreground/40">Layout</dt><dd>{order.layoutChoice}</dd></div>
                <div className="flex justify-between"><dt className="text-foreground/40">Code Unlock</dt><dd>{order.codeUnlocked ? 'Included' : 'Not included'}</dd></div>
                <div className="flex justify-between"><dt className="text-foreground/40">Status</dt><dd className={order.status === 'PAID' || order.status === 'COMPLETED' ? 'text-foreground' : 'text-foreground/60'}>{order.status === 'PAID' || order.status === 'COMPLETED' ? 'Paid (in escrow)' : 'Awaiting payment approval'}</dd></div>
          ...
          <div className="grid grid-cols-2 gap-4 mb-8">
            <Link href="/editor" className="p-6 rounded-3xl bg-white/[0.03] border border-white/[0.08] hover:border-white/20 transition-all text-center">
              <PenTool className="mx-auto mb-2 text-foreground/70" size={22} />
              <h3 className="font-display font-bold mb-1">Open Editor</h3>
              <span className="text-xs text-foreground/30">Modify your site instantly</span>
            </Link>
          ...
          <div className="p-6 rounded-3xl bg-white/[0.02] border border-white/[0.06] flex items-center gap-4 text-left">
            ...
            <div>
              ...
              <p className="text-sm text-foreground/30">Funds are held until you confirm satisfaction. Full refund available within 48 hours.</p>
            </div>
          </div>
        </div>
      </div>
